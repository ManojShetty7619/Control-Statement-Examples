...
public class Notepad extends JFrame implements ActionListener {// like it can use all the
	private JTextArea txt = new JTextArea(); // basically i had to make it local so that it could be used in more than 1
												// method yh i see thnx for explaining :d
	...
	private JMenu newMenu(String title, String[] elements) {
		...
		for (String element : elements) { // u understand?yes :d
			...
			menu.add(menuitem); // uses the add method in the JMenu class for our menu to add them menuitems yh
								// ok :d
	...
	private Notepad() {
		...
		try {
			UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName()); // basically gives it the system
																					// themeik
		...
		JScrollPane scroller = new JScrollPane(txt);// it has the txt(JTextArea) in it to select what its the container
													// fork
		add(scroller); // adds the scroller which has the text area in it.ok
		// Now this is where we have to make a menubar for files, i'll show you a nice
		// way to do menus.ok
	...
	// This shit here is for later to make the buttons and shit work. i know :d fk
	// don't delete this class sent over msn soon :d
	public void actionPerformed(ActionEvent actionEvent) {
		...
		if (cmd.equals("Save")) { // If the button pressed has the text "Save" on it do something inside.
			...
			int option = chooser.showSaveDialog(this); // Shows the save dialog and is the option for what you've
														// clicked
			if (option == JFileChooser.APPROVE_OPTION) { // if you've pressed the ok or save button or w/e do
															// somethingok
				...
				try {
					...
					// ^ Basically the bufferedwriter is something used for writing to a file along
					// with filewriter
					// yeah and the chooser.getSelectedFile().getAbsolutePath() basically finds the
					// place in the filechooserik
					// and writes to it.
					buf.write(txt.getText()); // basically this gets the text in the text area and writes it to the file
					setTitle(chooser.getSelectedFile().getName()); // this basically gets the file name in the chooser.
																	// rofl thisi s easy lol :d thnx, i told you it
																	// wasnt hard :Pyh
			...
			if (option == JFileChooser.APPROVE_OPTION) {
				try {
					...
					// Problem is i havent used file filter in a while so i mite do something wrong.
					// ok
